...
* and Asger Hobolth, Arno Siri-Jégousse, Mogens Bladt (2019): 
...
t = -Te,
\end{equation*}
where $e$ is a $p \times 1$ vector with one in each entry. By disregarding the absorbing state, we also have to reduce the dimension of the initial distribution $\pi= (\pi_1,…,\pi_p)$. In this new set-up, $\pi$ is now a $1 \times p$ vector that has to sum to one.\
