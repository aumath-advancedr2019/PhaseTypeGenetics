---
title: "PhaseTypeGenetics - An overview"
...
This vignette gives an overview of the package `PhaseTypeGenetics` without showing the mathematical details. For a more thorough explanation including mathematical proofs, we refer to the second vignette *PhaseTypeGenetics_Mathematical_Details*.
...
For $n=4$, the time to the most recent common ancestor is phase-type distributed with initial distribution $\pi = (1,0,0)$ and sub-intensity rate matrix
\begin{equation*}
T = \begin{pmatrix}
-6 & 6 & 0\\
0 & -3 & 3\\
...
summary(obj)
```
For $\theta =2$, the number of segregating sites plus one is discrete phase-type distributed with initial distribution $\pi = (1,0,0,0)$ and sub-transition probability matrix
\begin{equation*}
P = \begin{pmatrix}
0.4 & 0.3 & 4/30 & 2/30\\
0 & 0.5 & 2/9 & 1/9\\
...
Let $T$ be a sub-intensity matrix, then for any $a$ greater than the maximum of the absolute value of the diagonal entries of $T$, 
\begin{equation*}
I+a^{-1}T
\end{equation*}
is a sub-transition probability matrix. The function is implemented so that `discretization` applied to a `contphasetype` object with initial distribution $\alpha$ and sub-intensity matrix $T$ and a given $a$ returns a `discphasetype`object with initial distribution $\alpha$ and sub-transition probability matrix $I+a^{-1}T$.
...
\end{equation*}
The function is implemented so that `discretization` applied to a `contphasetype` object with initial distribution $\alpha$ and sub-intensity matrix $T$ and a given $\lambda$ returns a `discphasetype`object with initial distribution $\alpha$ and sub-transition probability matrix $(I-\lambda^{-1}T)^{-1})$.
...
If the given `a` is greater than the maximum of the absolute value of the diagonal entries of the sub-intensity matrix of `object`, `discretization(object = object,a = a)` performs the first transformation of the object.
...
\begin{align*}
\mathbb{P}(\tau= n) = \pi T^{n-1} t.
\end{align*}
for $n \geq 1$, where $t=(I-T)\boldsymbol{e}$.
\begin{align*}
...
f(u) = \pi e^{Tu}t
\end{align*}
for $u \geq 0$, where $t = -T\boldsymbol{e}$.
...
Then the distribution function for $\tau$ is given by
\begin{align*}
 F_{\tau}(u) = 1- \pi e^{Tu}\boldsymbol{e}
\end{align*}
for $u \geq 0$.
...
dist_Mat <- matrix(nrow = 6, ncol = length(t_vec))

## Now we can compute the distributions
dist_Mat[2,] <- dphasetype(T_MRCA$n5, t_vec)
dist_Mat[3,] <- dphasetype(T_MRCA$n10, t_vec)
dist_Mat[4,] <- dphasetype(T_MRCA$n20, t_vec)
dist_Mat[5,] <- dphasetype(T_MRCA$n50, t_vec)
dist_Mat[6,] <- dphasetype(T_MRCA$n100, t_vec)

## For n=2, the initial distribution is equal to 1 and 
## the sub-intensity rate matrix is T_Mat = -1. Hence,
## the distribution is given by
dist_Mat[1,] <- exp(-t_vec)
...
points(t_vec, dist_Mat[2,], type = "l")
points(t_vec, dist_Mat[3,], type = "l")
points(t_vec, dist_Mat[4,], type = "l")
points(t_vec, dist_Mat[5,], type = "l")
points(t_vec, dist_Mat[6,], type = "l")
...
## Defining the vector of quantiles
t_vec <- seq(0,15, by=0.1)

## and the matrix holding all distributions
dist_Mat <- matrix(nrow = 6, ncol = length(t_vec))

## Now we can compute the distributions
dist_Mat[2,] <- dphasetype(T_Total$n5, t_vec)
dist_Mat[3,] <- dphasetype(T_Total$n10, t_vec)
dist_Mat[4,] <- dphasetype(T_Total$n20, t_vec)
dist_Mat[5,] <- dphasetype(T_Total$n50, t_vec)
dist_Mat[6,] <- dphasetype(T_Total$n100, t_vec)

## For n=2, the initial distribution is equal to 1 and 
## the sub-intensity rate matrix is T_Mat = -1/2. Hence,
## the distribution is given by
dist_Mat[1,] <- exp(-t_vec/2)/2
...
K=\begin{pmatrix}S\otimes T & S\otimes t & s\otimes T\\
                  ...
                  0 & 0 & T\end{pmatrix},
\end{equation*}
and $s=\boldsymbol{e}-S\boldsymbol{e}$, $t=\boldsymbol{e}-T\boldsymbol{e}$ are the exit probability vectors of $S$ and $T$, respectively. \
This theorem is implemented in the `minima` and `maxima` functions. They take two phase-type objects as input and if both of these are of class `discphasetype`, then they each output an object of class `discphasetype` with parameters calculated from the parameters of the given objects in accordance with the above theorem.
...
\begin{equation*}K=\begin{pmatrix}S\oplus T & I\otimes t & s\otimes I\\
                  ...
                  0 & 0 & T\end{pmatrix},
\end{equation*}
and $s=-S\boldsymbol{e}$ as well as $t=-T\boldsymbol{e}$ are the exit rate vectors from $S$ and $T$, respectively, and $\oplus$ is the Kronecker sum given by $A \oplus B =A\otimes I_{\text{dim}(B)} + I_{\text{dim}(A)}\otimes B$.\ 
This result is implemented in the `minima` and `maxima` functions. They take two phase-type objects as input and if both of these are of type `contphasetype`, then they each output an object of class `contphasetype` with parameters calculated from the parameters of the given objects in accordance with the above theorem. 
...
In general, the Laplace transform of a non-negative random variable $\tau$ is defined as 
\begin{equation*}
...
\begin{align*}
\frac{\partial^n}{\partial s^n} L_{\tau}(s) = \int_{0}^{\infty} (-u)^n e^{-su} f_{\tau}(u) du.
\end{align*}
When evaluating this derivative at $s=0$, we see that
\begin{align*}
\frac{\partial^n}{\partial s^n} L_{\tau}(0) =(-1)^n\int_{0}^{\infty} u^n f_{\tau}(u) du.
\end{align*}
We recognize this last integral as the definition of the $n$'th moment of $\tau$, i.e.
\begin{align*}
...
\begin{align*}
\frac{\partial^n}{\partial s^n} L_{\tau}(0) = (-1)^n n! \pi (-T)^{-n}e.
\end{align*}
...
This theorem is implemented in the function `phsum`. It takes two phase-type distributed objects as input and if both of these are of type `discphasetype`, then it outputs an object of class `discphasetype` with parameters calculated from the parameters of the given objects in accordance with the above theorem.
...
This theorem is also implemented in function `phsum`. It takes two phase-type distributed objects as input and if both of these are of class `contphasetype`, then it outputs an object of the `contphasetype`-class with parameters calculated from the parameters of the given objects in accordance with the above theorem.
...
```
We can also compute the phase-type representation of the number of segregating sites $S_{\text{Total}}$ for a sample of size $n=4$ and the same mutation rate $\lambda = 1$
```{r}
SiteFrequencies(n=4, lambda=1, nSegSites=TRUE)

```
Finally, we find the phase-type representation of the tail statistic $S_{2+}$ for the same sample size $n=4$ and the same mutation rate $\lambda = 1$
```{r}
