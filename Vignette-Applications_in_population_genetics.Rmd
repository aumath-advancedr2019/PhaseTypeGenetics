...
\end{pmatrix}.
\end{equation*}
As one of the rewards is zero, we have a defect size of $\pi_{i,d+1}=0$, where $d=3$.\
...
For $n=4$, we have seen that the total length of branches giving rise to singletons is phase-type distributed with initial distribution $\pi_i = (1,0,0)$, subintensity rate matrix 
\begin{equation*}
T_i = \begin{pmatrix}
-1.5 & 1.5 & 0 \\
0 & -1.5 & 1  \\
...
\end{pmatrix}.
\end{equation*}
and defect size $\pi_{i,d+1}=0$, where $d=3$. From above we have that the number of singletons plus one is again phase-type distributed with initial distribution $\pi_i = (1,0,0)$ and subintensity rate matrix 
\begin{equation*}
...
\end{pmatrix}.
\end{equation*}
Hence, the number of segregating sites plus one is also phase-type distributed with initial distribution $\pi_i = (1,0,0,0)$ and subintensity rate matrix 
\begin{equation*}
