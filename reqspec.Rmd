...
* rphasetype: random numbers from a phase type distribution with initial distribution initDist and subintensity matrix Tmat
* qphasetype : The quantile function 
* LaplacePhaseType: the Laplace transform for a phase-type distribution with initial distribution initDist and subintensity matrix Tmat
...
We will define the class phasetype, in order to make it possible for the user to use the standard functions mean() and variance().


Furthermore, we will include a dataset which is composed of an initial distribution, a subintensity matrix and several reward vectors to be able to show examples of applications.
